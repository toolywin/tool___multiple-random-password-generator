...
function run_live_mode_pwd_generator(randomize) {
	let ids = ['uppercase', 'lowercase', 'number', 'symbol', 'length', 'quantity'];
	randomize = typeof randomize!= 'undefined' && randomize ? true : false;
	...
	if( $('#live_mode').is(':checked') )
	{
		...
		if( !randomize ) {
			...
			if( !char_used || $('#length').val()<5 || $('#length').val()>MAX_LENGTH || $('#quantity').val()<1 || $('#quantity').val()>MAX_QUANTITY ) {
				return;
			}
		}

		// ok
		$('.pwd_item').eq(0).find('input').val('');
		...
		let input_length, input_upper, input_lower, input_number, input_symbol,
			input_quantity = randomize ? RANDOM_QUANTITY : $('#quantity').val();

		for(let i=0; i<input_quantity; i++) {
			html_appended = html;

			input_length   = $('#length').val();
			input_upper    = $('#uppercase').is(':checked');
			input_lower    = $('#lowercase').is(':checked');
			input_number   = $('#number').is(':checked');
			input_symbol   = $('#symbol').is(':checked');

			if( randomize ) {
				input_length   = getRandomInt(5, 64);
				input_upper    = getRandomInt(0, 1);
				input_lower    = getRandomInt(0, 1);
				input_number   = getRandomInt(0, 1);
				input_symbol   = getRandomInt(0, 1);
			}

			new_pwd = randomString(input_length, input_upper, input_lower, input_number, input_symbol);

			$('.pwd_container').append( $(html_appended).prop('outerHTML') );

			new Typed($('.pwd_container').find('input').last().get(0), {
...
document.addEventListener("DOMContentLoaded", function(e) {
	...
	$(document).on('click', '.btn_pwd_new', function() {
		let s = randomString($('#length').val(), $('#uppercase').is(':checked'), $('#lowercase').is(':checked'), $('#number').is(':checked'), $('#symbol').is(':checked'));

		//$(this).closest('div').find('input').val( s );
		//$(this).closest('div').find('input').attr('data-value', s);

		// typewriter effect to show new string
		new Typed($(this).closest('div').find('input').get(0), {
			strings: [s, s],
			typeSpeed: 0,
		});
	...
	$(document).on('click', '.pwd_meter_bar', function() {
		...
		let second = zxcvbn( $(this).closest('.pwd_item').find('input').val() ).crack_times_seconds.offline_slow_hashing_1e4_per_second,
			second_display = '';
	...
	$(document).on('click', '.btn_pwd_qrcode', function() {
		...
		const qrcode = kjua({
			text: $(this).closest('div').find('input').val(),
		...
		myPopover._config.content = $('<p class="text-center">' + $(this).closest('div').find('input').val() + '<br/><br/></p>').append(qrcode);
		myPopover.setContent();
	});

	// handler memorable words
	$(document).on('click', '.btn_pwd_word', function() {
		const myPopover = bootstrap.Popover.getInstance( $(this).find('i').get(0), {placement: 'bottom', trigger: 'focus', html: true} );

		myPopover._config.content = $('<p class="text-center">' + $(this).closest('div').find('input').val() + '<br/><br/></p>').append( getRandomWord($(this).closest('div').find('input').val()) );
		myPopover.setContent();
	});
	...
	if( localStorage.getItem("term")==null || !Boolean(localStorage.getItem("term")) )
	{
